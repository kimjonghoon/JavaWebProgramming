...
<meta name="Keywords" content="static,클래스 레벨,class-level,싱글턴 패턴,singleton pattern,초기화 순서,Initialization order" />
<meta name="Description" content="static 키워드를 사용해 클래스 레벨의 변수와 메소드를 선언합니다" />
...
<p>
클래스가 메모리에 로딩될 때 static 변수와 static 메소드를 위한 공간이 할당된다.
한번 메모리에 공간이 할당되면 인스턴스 변수와 달리 객체가 생성될 때마다 메모리 공간이 할당되지 않는다.
static 메소드 안에서는 인스턴스 변수를 쓸 수 없다.
...
public class Student {
    ...
    public void absent() {
        this.absent++;
        <strong>Student.penaltyBin</strong> += 3000; //인스턴스 메소드에서 static 변수를 쓸 수 있다.
    }
    //..중간 생략..
}    
</pre>
...
CookTable의 static 변수 instance가 초기화 되기 위해 CookTable 객체가 힙에 생성되고 레퍼런스가 instance에 할당된다.
이 값은 공개된 getInstance() 메소드를 통해 얻을 수 있다.
...
인스턴스 변수든 정적 변수든 메모리에 로딩될 때 초기값이 없다면, 불린은 false, 숫자 타입에는 0으로, 레퍼런스 타입은 null로 초기화된다.
