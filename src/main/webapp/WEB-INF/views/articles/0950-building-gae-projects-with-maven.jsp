...
<head>
<title>메이븐으로 구글 앱 엔진 개발</title>
<meta name="Keywords" content="java,자바,google app engine,구글 앱 엔진,google cloud,구글 클라우드,maven,cloud sdk,eclipse,git,guestbook,방명록,git repository," />
<meta name="Description" content="메이븐으로 구글 앱 엔진 프로젝트 개발" />
...
<p>
서비스 신청 후 구글은 1년간 사용할 수 있는 300달러 무료 이용권을 제공한다.
1년간 300달러에 해당하는 서비스를 체험하는 것은 무료다.
300달러를 소진하거나 1년이 지나면 구글은 서비스를 계속 사용할지를 묻는다.
서비스를 계속 사용하겠다는 확답을 주면, 구글은 서비스 비용을 0으로 초기화하고 이후 발생한 서비스 비용은 신청 시 등록한 카드로 결제한다.
사용자는 무료 이용권을 사용하는 동안 자신에 맞는 서비스를 정하고 예산을 수립할 수 있는 역량을 키워야 한다.
</p>

<h2>프로젝트 생성</h2>

<p>
신청을 완료하고 다시 <a href="https://cloud.google.com">https://cloud.google.com</a>을 방문하면 '콘솔로 이동' 버튼이 보인다.
버튼을 클릭하여 콘솔 화면으로 이동한다.
</p>
...
<p>
윈도에서는 자바 8을, 우분투에서는 자바 11을 설치한다.
자바 8이 기본 환경이지만 우분투에서는 Cloud SDK글 설치할 때 자바 8이 같이 설치되므로 자바 11을 설치해도 된다.
윈도에 자바를 설치하는 방법은 아래 링크를 참조한다.<br />
<a th:href="@{/java/JDK-Install}">JDK 설치</a>
</p>

<h3>메이븐</h3>

<p>
<a th:href="@{/spring/building-java-projects-with-maven#Install-Maven}">메이븐 설치</a>를 참조하여 최신 버전을 설치한다.
<p>
...
존 지정은 인스턴스가 생성될 지역을 설정하는 것으로, 주로 서비스되는 지역과 가까운 곳을 지정하는 게 좋다.<br />
존에 대한 정보는 다음 주소를 참조한다.<br />
<a href="https://cloud.google.com/compute/docs/regions-zones/">https://cloud.google.com/compute/docs/regions-zones/</a>
</p>
...
압축을 풀고 생성된 디렉터리를 적당한 곳으로 옮긴 후 Git의 bin 경로를 PATH 환경변수에 추가한다.
</p>
...
<p>
<a href="http://www.eclipse.org">http://www.eclipse.org</a>에서 이클립스 인스톨러 파일을 내려받는다.
...
로컬 테스트와 달리 다음과 같은 서버 에러를 만날 수 있다.
