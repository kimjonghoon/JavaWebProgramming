...
<head>
<title>제네릭</title>
<meta name="Keywords" content="java,자바,Generic,제네릭,Generic Method,제네릭 메소드,Bounded Type Parameters,제약된 타입 파라미터,Type Inference,타입 추론,Target Types,타겟 타입,Wildcards,와일드 카드,Upper Bounded Wildcards,상한 제약 와일드카드,Unbounded Wildcards,제약없는 와일드카드,Lower Bounded Wildcards,하한 제약 와일드카드,Wildcards and Subtyping,와일드카드와 서브 타입,Type Erasure,타입 제거,Bridge Methods,브릿지 메소드,Non-Reifiable Types,구체화할 수 없는 타입,heap pollution,힙 오염,Varargs Methods,가변인자 메소드,Restrictions on Generics,제네릭의 제한," />
<meta name="Description" content="제네릭" />
...
Box&lt;Integer&gt; 와 Box&lt;Double&gt;은 Box&lt;Number&gt;의 서브 타입이 아니기 때문이다. 
...
<p>
타입 삭제 후, 메소드 시그니처가 일치하지 않는다.<br />
Box의 메소드는 set(Object)가 되고 IntBox의 메소드는 set(Integer)이 된다.<br />
따라서 IntBox.set(Integer)은 Box.set(Object)를 오버라이딩하지 않는다.<br />
다형성을 유지하기 위해 자바 컴파일러는 브리지 메소드를 생성하여 서브 타입이 기대한 대로 작동하도록 한다.<br />
이렇듯 제네릭을 상속하거나 구현하는 클래스를 컴파일할 때, 컴파일러는 타입 삭제 프로세스의 일부로서 브리지 메소드라는 복합 메소드를 생성할 수 있다.<br />
IntBox 클래스를 위해, 컴파일러는 set(Integer)를 위한 브리지 메소드를 생성한다.<br />
...
파라미터화된 리스트의 배열이 허용된다면, ArrayStoreException 예외를 던져야 할 상황이 오더라도 던지지 못할 것이다.
</p>
