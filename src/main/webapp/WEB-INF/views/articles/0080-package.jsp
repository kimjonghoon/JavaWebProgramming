...
<p class="floatstop">
패키지는 말 그대로 꾸러미이다.
쓰레기 분류 수거할 때에 사용하는 꾸러미를 생각해 보라.
자바는 비슷한 기능이나 비슷한 성격을 가진 클래스들을 관리하기 위해서 패키지를 제공한다.
자바 API는 모두 특정 패키지에 속해있다.(주로 java로 시작하는 이름의 패키지이다.)
이제 우리가 만드는 클래스가 특정 패키지에 속하게 하는 방법을 알아본다.
</p>
...
<p>
첫번째 라인의 <strong>package javabank;</strong> 의해서 Account 클래스는 javabank패키지에 속하게 된다.<br />
패키지를 선언한 자바 소스를 컴파일 할 때는 반드시 -d 다음에 한칸 띄고 바이트 코드가 생성될 
디렉토리를 지정해 주어야 한다. 만일 -d 옵션을 생략한다면 바이트 코드는 패키지가 적용되지 않는다.<br />
...
서브 디렉토리를 만들고 해당 소스를 그곳에 위치하도록 한다.(대부분 이렇게 소스를 관리한다.)
</p>

<dl class="note">
<dt>패키지 이름은 일반적으로 도메인 이름의 역순으로 짓는다.</dt>
<dd>
대개 패키지 이름은 도메인 이름의 역순으로 짓는다.<br />
또한, 패키지 이름에 .(도트)가 2개 이상 포함할 것을 권장한다.<br />
...
<p>
다시 Account.java로 돌아오자.
모든 준비가 되었다면 소스 파일이 있는 C:/javaApp/bank/src/javabank로 이동한 후 아래와 같이 컴파일한다.
</p>
...
<p>
-d 옵션다음의 경로 구분자는 윈도우 시스템이라도 /(슬래시)를 사용할 수 있다.
물론 윈도우 시스템의 경로 구분자 \(역슬래시)도 사용할 수 있다.
컴파일 후 C:/javaApp/bank/bin 디렉토리로 이동해서 Account의 바이트 코드가 생겼는지 확인한다.
</p>
...
<p>
bin 디렉토리에는 Account.class 파일이 보이지 않고, 대신 javabank 디렉토리가 보인다.<br />
Account.class 파일은 javabank 서브 디렉토리에 있다.<br />
...
<p>
JVM은 C:/javaApp/bank/bin디렉토리에 javabank.Account클래스가 있다고 이해한다.
패키지가 적용된 클래스는 패키지명.클래스명이 완전한 클래스명(Fully Qualified Class Name:FQCN)이다.
("길동"이 아닌 "홍길동"이 완전한 이름이듯이)
다른 클래스에서 Account 클래스를 사용하려면 코드에서 javabank.Account로 써야 한다.
(자바 문서에서 "클래스"란 용어는 문맥에 따라 바이트 코드를 의미할 때가 있고 클래스 소스 파일을 의미할 때가 있다.) 
클래스를 실행할 때도 마찬가지이다.
Account 클래스를 실행하기 위해서는 C:/javaApp/javabank/bin 디렉토리에서 
java javabank.Account로 실행해야 한다.
다시 한번 강조해서, JVM은 bin 디렉토리에 javabank.Accont 클래스가 있다고 이해하기 때문이다.
</p>
...
<p>
만약 클래스가 없는 디렉토리에서 실행하려면 어떻게 해야 할까?
이 경우 JVM에게 바이트 코드가 어디에 위치하는지 알려주어야 하는데,
자바 인터프리터(java.exe)의 classpath<a href="#comments"><sup>1</sup></a>옵션을 사용하면 된다.
아래는 C:/Program Files 디렉토리에서 javabank.Account를 실행하는 방법을 보여주고 있다.
</p>
...
<p>
-classpath다음에 한 칸 띄고 실행할 바이트 코드가 있는 곳을 절대 경로 또는 상대 경로로 지정한다.
위에서는 classpath 값을 절대 경로로 지정하고 있는데 상대 경로로 지정한다면 다음과 같다.
</p>
...
<p>
다음은 새로 만들 Log클래스<a href="#comments"><sup>3</sup></a>이다.
</p>
...
<p>
다음으로 기존 Accont클래스가 Log클래스를 사용하도록 수정한다.
Log클래스는 net.java_school.commons 란 패키지가 적용되었으므로 FQCN은 
net.java_school.commons.Log 이므로 소스에서도 그대로 써야 한다.
</p>
...
<p>
우리가 만든 자바 프로그램은, javabank.Account와 net.java_school.commons.Log 클래스로 구성되어 있다.
이 두 클래스의 위치가 다르므로 자바 인터프리터의 classpath옵션으로 두 클래스의 위치를 지정해야 한다.
classpath옵션이 사용되면 JVM은 관련 클래스를 classpath로 설정된 곳에서만 찾는다.
그래서 현재 디렉토리를 의미하는 .(도트)가 클래스 패스에 포함되어야 한다.
</p>

<h3>외부 자바 라이브러리를 자신의 프로그램에 추가하는 방법</h3>

<p>
대부분 외부 자바 클래스는 jar형태의 압축파일로 제공된다.
Log클래스를 다른 자바 프로그래머가 사용하도록 배포하려면 jar.exe 를 이용한다.
Log바이트 코드가 있는 디렉토리로 이동하여 다음 멸령을 수행한다.
</p>

<div class="cmd-header">&nbsp;</div>
<pre class="cmd">C:\javaApp\commons\bin&gt;jar cvf java-school-commons.jar .</pre>
...
1단계 접근 제어는 접근자가 클래스 선언에 쓰일 때이다.
</p>

<h3>public 접근자가 Account의 클래스 선언에 쓰인 경우</h3>
...
<h3>package private 접근자가 Account의 클래스 선언에 쓰인 경우</h3>
...
<tr>
	<th class="table-in-article-th" colspan="2">1단계 접근 제어 : 접근자가 클래스 선언에 쓰일 경우</th>
</tr>
<tr>
	<td class="table-in-article-td">public</td>
	<td class="table-in-article-td">
	모든 패키지의 클래스에서 참조할 수 있다. 
	</td>
</tr>
<tr>
	<td class="table-in-article-td">package private</td>
	<td class="table-in-article-td">
	같은 패키지에 있는 클래스에서만 참조할 수 있다.
	</td>
</tr>
</table>
...
<pre class="prettyprint">
<strong>package example;</strong>//Account에 적용된 javabank와 다른 패키지
...
그림에서 example 패키지에 있는 BankSystem는 javabank패키지의 모든 클래스를 접근할 수 있다.
이 말은 위의 BankSystem소스에서 볼수 있듯이 javabank.Account account;와 같이 변수 선언의 
데이터 타입으로 사용할 수 있고 파라미터 타입으로 javabank.Account를 쓸 수 있다는 의미이다.
import문을 이용하면 클래스 바디에서 javabank.Account를 클래스 바디에서 Account로 줄여 쓸 수 있다.
</p>
...
public class BankSystem {
	...
	public void deposit(<strong>Account</strong> account, double amount) {
...
<tr>
	<td class="table-in-article-td">public</td>
	<td class="table-in-article-td">모든 패키지에서 접근 가능하다.</td>
</tr>
<tr>
	<td class="table-in-article-td">protected</td>
	<td class="table-in-article-td">
	같은 패키지에서만 접근 가능하다.
	단, 다른 패키지에 속해있는 자식 클래스에서 부모클래스의 protected접근자로 지정된 멤버는 접근 가능하다.
	</td>
</tr>
<tr>
	<td class="table-in-article-td">package private</td>
	<td class="table-in-article-td">같은 패키지에서만 접근 가능하다.</td>
</tr>
<tr>
...
<p>
자바에서 필드(Fields)란 클래스 몸체에서 메소드가 아닌 모든 것을 의미한다.
멤버(members)는 객체의 속성을 저장하는 변수나 객체의 행위에 해당하는 메소드를 의미한다.
위의 표를 이해하기 쉽게 풀어 설명하도록 한다.
필드나 메소드에 적용되는 가장 쉬운 접근자는 public과 private이다.
public이면 모든 패키지에서 접근할 수 있다.
private이면 외부에서 접근할 수 없다.
package private 접근자는 같은 패키지에서만 접근할 수 있다.
protected는 package private 접근자보다 접근 허용범위가 더 넓다.
(사실 protected에 대해서는 상속을 공부한 후에 아래 설명을 다시 보는 게 낫다.)
...
이를 캡슐화라고 하는데 외부에서 볼 때 객체의 꼭 필요한 부분만 볼 수 있도록 해준다.
</p>
...
<p>
(getter,setter은 아래 예제를 보고 이해하자.)
웹사이트의 회원 클래스를 캡슐화하여 구현해 보자.
</p>
...
<h2 id="Install-Eclipse">이클립스 설치</h2>

<p>
다음 장부터 예제는 이클립스를 사용하기로 한다.
이클립스를 사용하면 위에서 실습했던 디렉토리 구조로 소스를 관리하게 되면서
클래스 패스에 대해서도 크게 신경쓰지 않아도 된다.

<a href="http://www.eclipse.org/downloads/">http://www.eclipse.org/downloads/</a>
에서 Eclipse IDE for Java EE Developers 선택하여 다운로드한다.
압축을 풀면 생기는 eclipse 폴더를 원하는 곳에 복사하면 설치가 끝난다.
여기서는 eclipse 폴더를 C:/ 에 붙여넣기했다고 가정하고 설명한다.
</p>
...
위 그림과 같이 워크스페이스를 디폴트로 보여지는 디렉토리로 선택하지 않도록 한다.
또한 Use this as the default and do not ask again 에 체크 하지 말고 그대로 둔다.
서로 다른 성격의 프로젝트를 관리하려면 워크스페이스를 여러개 두면 편리한 경우가 있을 수 있기 때문이다.
...
<h4>워크벤치(workbench)</h4>
<p style="padding-left: 14px;">
이클립스에서 보이는 윈도우 전체를 말한다.
윈도우는 크게 4개로 나뉘어지는데 이 분리된 영역을 뷰(view)라고 부른다.
</p>

<h4>perspective</h4>
<p style="padding-left: 14px;">
사전적 의미는 "관점"이다.
뷰를 모두 포함해서 퍼스펙티브(perspective)라고 부른다.
위 화면은 Java 퍼스펙티브이다.
이 퍼스펙티브는 자바 프로그램을 개발할 때 필요한 뷰들로 구성된다.
퍼스펙티브를 변경하려면 오른쪽 상단의 Open Perspective 메뉴바 버튼을 이용한다.
</p>

<h4>Package Explorer 뷰</h4>
<p style="padding-left: 14px;">
...
<p>
뷰의 위치는 마우스로 원하는 위치로 변경할 수 있다.
하지만 익숙해 질때까지는 그대로 두는 게 좋다.
</p>

<h3>자바 예제</h3>

<p>
먼저 Java Perspective 인지 확인한다.
이클립스에서는 자바 소스는 반드시 프로젝트에 속해야만 한다.
...
<p>
저장하면 컴파일을 따로 할 필요가 없다.
이클립스가 백그라운드에서 계속해서 컴파일을 해주기 때문이다.
에디터에서 바로 컴파일 에러를 확인할 수 있는 이유가 여기에 있다.
