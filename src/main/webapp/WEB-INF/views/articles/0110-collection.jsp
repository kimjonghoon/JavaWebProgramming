...
<p>
컬렉션(Collection)이란 같은 타입의 레퍼런스를 여러개 저장하기 위한 자바 라이브러리이다.<br />
"배열과 비슷한데 훨씬 더 편리하다." 라는 정도로 접근하자.<br />
다음 그림은 컬렉션 관련 주요 인터페이스의 계층관계를 보여 준다.<br />
<img src="<c:url value="/resources/images/collection-framework.png"/>" alt="Collection Framework" /><br />
...
<p>
예제는 Set 인터페이스의 사용법을 보여 주고 있다.<br />
HashSet을 생성하고 Set인터페이스의 add 메소드를 사용하여 이름을 추가한다.<br />
양효선은 중복 추가를 시도하고 있는데 Set은 중복을 허용하지 않으므로 추가되지 않는다.<br />
System.out.println(set);로 확인할 수 있다.<a href="#comments"><sup>2</sup></a><br />
System.out.println(set);다음에 기존의 HashSet을 TreeSet으로 처리한다.<br />
...
<h6 class="src">SetExample.java - 제네릭을 사용</h6>
<pre class="prettyprint">
package net.java_school.collection;
...
<p>
예제에서 다룬 컬렉션 클래스를 자바 문서에서 찾아보면 클래스 선언에 &lt;E&gt;, &lt;T&gt;, &lt;K, V&gt;를 볼 수 있다.<br />
...
public class ArrayListExample {
	...
	public static void main(String[] args) {
		...
		String hong = <strong>(String)</strong> a.get(1);//캐스팅 필요하다.
		System.out.println(hong);
		
		//모든 요소를 출력하려면
		for (<strong>Object</strong> name : a) {//자바 5에 도입된 '확장 for 문'
...
제네릭을 사용하면 특정 타입 레퍼런스만 저장하게 되므로 값을 가져올 때 타입 캐스팅이 필요없다는 사실에 주목해야 한다.<a href="#comments"><sup>3</sup></a>
</p>

<h6 class="src">ArrayListExample.java - 제네릭을 사용</h6>
<pre class="prettyprint">
package net.java_school.collection;
...
public class ArrayListExample {
	...
	public static void main(String[] args) {
		...
		String hong = <strong>a.get(1);</strong>//캐스팅이 필요없다.
		System.out.println(hong);
...
public class MapExample {
	public static void main(String args[]) {
		...
		System.out.println(<strong>(String)</strong> map.get("4"));//캐스팅 필요
...
<h6 class="src">MapExample.java - 제네릭 사용</h6>
<pre class="prettyprint">
package net.java_school.collection;
...
Integer는 int에 대응하는 래퍼(Wrapper) 클래스이다.
Integer 타입의 키값을 주면 HashMap도 정렬이 된다.
</p>
...
현재는 Vector 대신에 ArrayList가 더 많이 사용되고 있다.<a href="#comments"><sup>4</sup></a>
</p>
...
public class VectorExample {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i &lt; 10; i++) {
			System.out.println(v.<strong>elementAt</strong>(i));//Object 타입 레퍼런스 반환
		}
	}
...
<h6 class="src">VectorExample.java - 제네릭 사용</h6>
<pre class="prettyprint">
package net.java_school.collection;
...
자바 프로그램의 설정 데이터를 위해 만든 자바 프러퍼티에는 비영어권을 위한 배려는 없다.<br />
프로퍼티 파일에서 한글은 자바에서 사용하는 <strong>유니 코드</strong>로 저장되어야 한다.
</p>

<h3>Enumeration 인터페이스</h3>

<p>
열거 형태로 저장된 객체를 처음부터 끝까지 차례로 조회하는데 유용한 인터페이스이다.<br />
메소드는 다음 2개가 전부이다.
</p>
...
<p>
Collection인터페이스의 iterator() 메소드는 Iterator를 반환한다.<a href="#comments"><sup>6</sup></a><br />
...
<h2>래퍼(Wrapper) 클래스</h2>

<p>
컬렉션은 레퍼런스만 담을 수 있고 기본 타입 값은 담을 수 없다.<br />
기본 타입 값을 컬렉션에 담으려면 래퍼 클래스를 이용해야 한다.<br />
모든 기본 타입에 대응하는 래퍼 클래스가 존재한다.<br />
기본 타입 값을 멤버 변수에 저장하고 값 주위로 값을 가공하는 메소드들이 감싸고 있다고 해서 래퍼(Wrap:감싸다)클래스라고 불린다.
</p>

<table class="table-in-article">
<tr>
	<th class="table-in-article-th">기본 타입</th>
	<th class="table-in-article-th">래퍼 클래스</th>
</tr>
<tr>
...
<p>
유리로 사면이 막힌 상자에 동전을 넣고 다시 밖으로 꺼내는 마술이 있다.<br />
비유하면 상자 안에 동전을 넣는 것이 오토박싱이고, 상자 밖으로 동전을 꺼내는 것이 오토언박싱이다.<br />
오토박싱(AutoBoxing)과 오토언박싱(AutoUnboxing)은 개발상 편의를 위해 만들어졌다.
</p>

<span id="comments">주석</span>
<ol>
	<li>컬렉션 클래스는 Set 인터페이스를 구현했거나 List 인터페이스를 구현했거나 Map 인터페이스를 구현한 것으로 나뉜다.</li>
	...
	<li>
	자바 5 이전 컬렉션 클래스에서 값을 추가하는 메소드는 모두 파라미터 타입이 Object이다.<br />
	모든 레퍼런스를 저장할 수 있다고 하여 마치 자바의 강점처럼 설명되었다.<br />
	하지만 다양한 타입 레퍼런스를 컬렉션에 저장하는 경우가 많지 않을 뿐 아니라, 저장하더라도 값을 가져올 때 필요한 타입 캐스팅이 귀찮은 작업이라는 사실을 경험으로 깨닫게 되었다.
	</li>

	<li>
	ArrayList와 Vector는 큰 차이가 있다.<br />
	Vector는 스레드 안전<sup>Thread Safe</sup>하다.<br />
	반면, ArrayList는 그렇치 않다.<br />
	스레드 안전과 스레드 안전하지 않음은 성능상에 차이가 크다.<br /> 
	따라서 스레드 안전함을 선택할 때는 그만한 이유가 있어야 한다.<br />
	대부분의 경우 스레드 안전하지 않음을 선택하는 것이 옳다.<br />
	참고로, JDBC에서 다루게 되는 사용자 정의 JDBC 커넥션 풀링 소스는 Vector를 사용한다.
	</li>

	<li>
	이때 이클립스로 실행하는 경우와 콘솔에서 실행하는 경우의 파일 위치가 다르다.<br />
	이클립스에서 실행하면 프로젝트 루트 디렉토리에 생성된다.<br />
	모호하다고 생각되면 FileOutputStream과 FileInputStream의 생성자에 파일 시스템의 전체 경로를 전달하면 확실해 진다.<br />
	<pre>new FileOutputStream("C:/java/Collection/test.properties"), new FileInputStream("C:/java/Collection/test.properties")</pre>
	</li>
	
	<li>
	Set 이나 List 인터페이스를 구현한 모든 클래스에는 iterator() 메소드를 가진다.<br />
	Set과 List 인터페이스는 모두 Collection 인터페이스를 상속하기 때문이다.<br />
	iterator() 메소드의 반환 타입은 Iterator이다.<br />
	물론 Iterator가 인터페이스이기 때문에, 실제로는 Iterator를 구현한 클래스로부터 생성된 객체가 반환된다.<br />
	하지만 반환 타입이 구체적으로 어떤 클래스인지 관심을 가질 필요가 없다.<br />
