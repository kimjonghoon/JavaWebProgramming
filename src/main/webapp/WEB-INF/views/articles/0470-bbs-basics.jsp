...
<p>
board-schema.sql 파일을 이용하려면, scott 계정으로 SQL*PLUS에 접속한 후 @ 입력하고 이어서 board-schema.sql 파일의 전체경로를 입력한다.
C:\에 board-schema.sql 파일이 있다면 다음과 같이 실행한다.
</p>
...
게시물의 목록을 보여주는 페이지
(단계적으로 페이지 분할, 페이지  이동 링크, 검색 기능을 추가한다)
	</td>
</tr>
<tr>
	<td class="table-in-article-td">write_form.jsp</td>
	<td class="table-in-article-td">게시글 입력 양식</td>
</tr>
<tr>
	<td class="table-in-article-td">BoardWriter.java</td>
	<td class="table-in-article-td">게시글을 테이블에 삽입하는 서블릿</td>
</tr>
<tr>
	<td class="table-in-article-td">view.jsp</td>
	<td class="table-in-article-td">게시물의 상세 정보를 출력</td>
</tr>
<tr>
	<td class="table-in-article-td">modify_form.jsp</td>
	<td class="table-in-article-td">수정 입력 양식</td>
</tr>
<tr>
	<td class="table-in-article-td">BoardModifier.java</td>
	<td class="table-in-article-td">게시글을 수정하는 서블릿</td>
</tr>
<tr>
	<td class="table-in-article-td">BoardDeleter.java</td>
	<td class="table-in-article-td">게시글을 삭제하는 서블릿</td>
</tr>
</table>

<p>
JSP와 서블릿을 복습하기 위해서, 화면이 필요하면 JSP를, 화면이 필요 없으면 서블릿을 채택했다.<br />
다음은 게시판 프로그램의 흐름이다.
</p>
...
<p>
게시판의 게시글 레코드가 10000개라면 list.jsp안의 while문은 10000번 실행된다.<br />
데이터가 많으면 목록에서 레코드를 모두 보여 주지 않고 레코드를 그룹으로 나누어 보여주는 기능이 필요하다.<br />
이러한 기능을 <strong>"페이지 분할 기능"</strong>이라고 부르기로 하자.
</p>
...
<p>
ROWNUM은 오라클의 가상 컬럼으로, 쿼리 결과에서 1부터 시작하여 순차적인 값을 가진다.<br />
ROWNUM을 WHERE 절의 조건에 쓰면 보여줄 그룹에 해당하는 레코드를 추출할 수 있다.<br />
list.jsp를 요청할 때 레코드 그룹 번호를 파라미터로 넘겨준다면 그룹에 해당하는 ROWNUM의 시작 레코드 번호와 마지막 레코드 번호를 구할 수 있다.<br />
list.jsp에 전달할 레코드 그룹 번호에 해당하는 파라미터를 curPage라 정하고 list.jsp를 아래 코드를 참조하여 수정한다.<br />
</p>
...
<p>
이제 주소창에서 http://localhost:8080/board/list.jsp?curPage=1을 요청하면 그룹 1의 레코드를 볼 수 있고,
http://localhost:8080/board/list.jsp?curPage=2를 요청하면 그룹 2의 레코드를 볼 수 있게 되었다.
하지만, 웹 브라우저의 주소창에서 curPage 파라미터를 고쳐가며 페이지를 이동하는, 그런 게시판은 없다.
게시판은 아래와 같은 페이지를 이동할 수 있는 링크를 제공한다.
</p>
...
페이지가 1부터 시작하므로 '마지막 페이지 번호'는 '총 페이지 수'와 같다.
총 페이지 수는 총 레코드 수를 페이지당 레코드 갯수인 10으로 나누면 계산된다.
...
<p>
이제 총 페이지 수, 즉 마지막 페이지 번호를 구했다.
이 시점에서 ROWNUM의 시작 레코드와 마지막 레코드를 구하는 코드를 좀 더 우아하게 고쳐 보겠다.
페이지 당 레코드 수를 저장하기 위한 변수 numPerPage를 선언하고, 아래 코드를 참조하여 list.jsp의 코드를 수정한다.
</p>
...
<ol>
	<li>총 레코드 수를 구한다.</li>
	<li>페이지당 보일 레코드 수를 결정하고 총 페이지 수를 구한다.</li>
	<li>첫 번째 레코드 번호와 마지막 레코드 번호를 구하고 레코드를 출력한다.</li>
	<li>페이지 이동 링크를 만든다.</li>
</ol>
...
<dd>
JSP에는 page란 내재객체<sup>Implicit Object</sup>가 있다.<br />
JSP가 서블릿으로 변환될 때 다음과 같은 코드가 생성되어 추가된다.<br />
...
<p>
오라클 11g부터 rank() 함수를 이용할 수 있다.<br />
...
<p>
이 함수를 이용하면 위의 게시판 목록에 해당하는 쿼리문을 조금 간단하게 줄일 수 있다.<br />
...
<h2>페이지 이동 링크 수 제한 기능</h2>

<p>
완벽하게 보이는 페이지 분할 기능에도 문제가 있다.<br />
레코드가 10000개이고 numPerPage가 10이라면 하단의 페이지 이동 링크는 [1] [2] [3] ...... [999] [1000] 식으로 1000개가 생긴다.<br />
1000개의 페이지 이동 링크는 웹 디자이너가 만든 예쁜 디자인을 헤집어 놓는다.<br />
페이지 이동 링크 역시 그룹으로 나누면 해결된다.<br />
이 기능을 <strong>'페이지 이동 링크 수 제한 기능'</strong>이라고 부르겠다. 
</p>
...
<h6 class="src">curPage가 속한 block 구하기</h6>
<pre class="prettyprint">
...
<h6 class="src">block에 속한 첫 번째 페이지 번호와 마지막 페이지 번호 구하기</h6>
<pre class="prettyprint">
...
인접한 페이지 그룹으로 이동할 수 있도록 링크는 다음 방법으로 생성할 수 있다.<br />
block이 1보다 크면 [이전] 링크를 생성하고 firstPage - 1인 페이지로 링크하고, 
block이 총 블록 수(마지막 블록 번호)보다 작다면 [다음] 링크를 생성하고 lastPage + 1인 페이지로 링크한다.<br />
이렇게 [이전] [다음] 링크를 사용하면 인접한 페이지 그룹으로 이동할 수 있다.<br />
[다음] 링크를 만들기 위해선 총 블록 수, 즉 마지막 블록을 구하는 코드가 추가로 필요하다.<br />
</p>

<h6 class="src">총 블록 수 구하기 (마지막 블록 번호 구하기)</h6>
<pre class="prettyprint">
...
그리고 pagePerBlock이 5라면 totalBlock은 3으로 계산된다.<br />
블록 3에 속하는 마지막 페이지 번호는 15로 계산되어 필요없는 [12] [13] [14] [15] 링크가 만들어지게 된다.<br />
불필요한 페이지 링크가 생성되는 않게 하려면, 마지막 블록에서 마지막 페이지 번호를 총 페이지 수(마지막 페이지 번호)로 설정해야 한다.<br />
...
<h3>최종 페이지 분할 기능 알고리즘</h3>
<ol>
	<li>블록 당 페이지 이동 링크 수를 정한다.</li>
	<li>총 블록 수를 계산한다.</li>
	<li>사용자가 요청한 페이지(현재 페이지)가 속한 블록을 계산한다.</li>
	<li>현재 페이지가 속한 블록에서 링크할 첫 번째 페이지와 마지막 페이지를 계산한다.</li>
	<li>현재 페이지가 속한 블록에서 불필요한 페이지를 제거한다.</li>
	<li>block &gt; 1이면 firstPage - 1로 이동하는 [이전] 링크를 만든다.</li>
	<li>루프 문을 사용해여 첫 번째 페이지부터 마지막 페이지까지 링크를 만든다.</li>
	<li>block &lt; totalBlcok이면 lastPage + 1로 이동하는 [다음] 링크를 만든다.</li>
</ol>

<h3>페이지 분할 기능 추가에 따른 게시판 컴포넌트 수정</h3>

<p>
list.jsp?curPage=5에서 view.jsp을 방문한 후 view.jsp에서 목록 링크를 클릭하면 list.jsp로 방문하게 된다.<br />
즉, 5페이지에서 상세보기를 보고 다시 목록으로 돌아오는데 1페이지로 돌아온다.<br />
5페이지에서 상세보기로 이동하고, 상세보기에서 목록 링크를 클릭하면 다시 5페이지로 돌아가야 한다.<br />
그렇게 하려면 '새 글쓰기를 처리하는 서블릿'을 제외한 게시판과 관련된 모든 컴포넌트에 curPage 파라미터를 전달하고, 
curPage를 전달받은 컴포넌트는 다른 컴포넌트로 이동하기 위한 링크의 쿼리 스트링에 이 파라미터를 추가해야 한다.<br />
/board/list.jsp 파일을 열고 아래를 참고하여 다음과 같이 링크의 쿼리 스트링을 수정한다.
</p>

<h6 class="src">list.jsp에서 view.jsp로 이동할 때 curPage 파라미터 전달</h6>
<pre class="prettyprint">
&lt;a href="view.jsp?no=&lt;%=no %&gt;<strong>&amp;curPage=&lt;%=curPage %&gt;</strong>"&gt;&lt;%=title %&gt;&lt;/a&gt; &lt;%= wdate.toString() %&gt;
</pre>

<h6 class="src">list.jsp에서 write_form.jsp로 이동할 때 curPage 파라미터 전달</h6>
<pre class="prettyprint">
&lt;a href="write_form.jsp?<strong>curPage=&lt;%=curPage %&gt;</strong>"&gt;글쓰기&lt;/a&gt;
</pre>
...
<h6 class="src">/board/view.jsp 수정</h6>
<pre class="prettyprint">
&lt;%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%&gt;
&lt;%@ page import="java.sql.*" %&gt;
...
<p>
/board/write_form.jsp 파일을 열고 목록으로 돌아가는 부분의 코드를 수정한다.<br />
폼 액션 속성값을 ../servlet/BoardWriter?curPage=&lt;%=curPage %&gt;로 고쳐선 안 된다.<br />
왜 그럴까? 새 글은 언제나 목록 첫 페이지로 이동해야 확인할 수 있기 때문이다.<br />
5 페이지에서 새 글을 등록했는데 다시 5페이지로 돌아간다면 자신이 작성한 새 글을 확인할 수 없다.<br />
따라서 새 글을 등록하면 목록 첫 페이지로 이동해야 한다.
</p>

<h6 class="src">/board/write_form.jsp 수정</h6>
<pre class="prettyprint">
&lt;%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%&gt;
&lt;%
...
<h6 class="src">/board/modify_form.jsp 수정</h6>
<pre class="prettyprint">
&lt;%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%&gt;
&lt;%@ page import="java.sql.*" %&gt;
...
<h6 class="src">BoardModifier 서블릿 수정</h6>
<pre class="prettyprint">
@Override
public void doPost(HttpServletRequest req, HttpServletResponse resp)
		throws ServletException, IOException {
...
<h6 class="src">BoardDeleter 서블릿 수정</h6>
<pre class="prettyprint">
@Override
public void doPost(HttpServletRequest req, HttpServletResponse resp)
		throws ServletException, IOException {
...
<p>
페이징 처리는 웹 프로그래머라면 반드시 정복해야 한다.<br />
구현과 충분한 테스트를 통해서 페이징에 대한 완벽한 이해를 해야 한다.<br />
</p>

<h2>검색 기능</h2>

<p>
게시글이 많아지면 필요해지는 기능이 검색 기능이다.<br />
...
<p>
list.jsp에 curPage외에 keyword 파라미터도 함께 전달해야 한다.<br />
전달받은 keyword 파라미터가 널인 경우 "" 문자로 바꾸어 주면, 나중에 널 체크를 하지 않아도 되어 편리하다.<br />
...
<p>
request.setCharacterEncoding("UTF-8"); 코드는 한글 검색을 위해 필요하다.<br />
이 코드는 getParameter() 메소드가 실행되기 전에 실행되야 한다.<br />
...
<p>
검색결과의 목록 페이지에서 하단의 5페이지 이동 링크를 클릭했을 때, 검색을 하지 않은 목록 5페이지가 보이면 안 된다.<br />
...
<p>
list.jsp를 방문한 후 검색을 테스트한다.<br />
검색 후 목록에서 상세보기로 이동한 후 다시 목록으로 돌아오면 검색된 목록으로 돌아오지 못한다.<br />
view.jsp 요청할 때 curPage 외에 keyword란 파라미터를 전달해야 다시 검색 목록으로 돌아갈 수 있다.<br />
...
<p>
view.jsp에는 curPage와 keyword 파라미터를 수신하고 list.jsp로 돌아갈 때는 이 파라미터를 이용하도록 코드를 수정한다.<br />
...
&lt;title&gt;상세보기&lt;/title&gt;
&lt;script type="text/javascript"&gt;
function goModify(no, curPage, <strong>keyword</strong>) {
...
<p> 
keyword = java.net.URLEncoder.encode(keyword,"UTF-8");코드가 필요한 이유는 HttpServletResponse 의 sendRedirect() 메소드의 아규먼트가 자바 문자열인데 자바 문자열과 URL 주소 문자의 인코딩이 서로 다르기 때문이다.<br />
URLEncoder의 encode() 메소드는 한글과 같은 문자에 대한 바이트 값을 얻을 수 있게 한다.<br />
<!-- Translates a string into application/x-www-form-urlencoded format using a specific encoding scheme. 
This method uses the supplied encoding scheme to obtain the bytes for unsafe characters. 
-->
이 코드가 우리가 원하는 방식으로 동작하려면 서버 설정을 건드려야 한다.<br />
CATALINA_HOME/conf/server.xml 파일을 열고 Connector 엘리먼트 중 port 속성값이 8080인 Connector 엘리먼트에 URIEncoding 속성을 다음과 같이 UTF-8인지를 확인한다.<br />
...
<p>
톰캣은 쿼리 스트링을 포함한 URL의 디폴트 캐릭터 인코딩으로 ISO-8859-1을 사용한다.<br />
즉, GET 파라미터의 인코딩은 ISO-8859-1을 사용한다.<br />
<strong>URIEncoding="UTF-8"</strong> 설정은 URL에 대한 캐릭터 인코딩을 UTF-8로 변경한다.<br />
이렇게 설정하면 값이 한글인 파라미터를 GET 방식으로 깨지지 않게 전송할 수 있다. <br />
톰캣을 재실행한다.<br />
아래 파일을 ROOT 애플리케이션의 도큐먼트베이스에 생성한다.
</p>

<h6 class="src">/sender.jsp</h6>
<pre class="prettyprint">
&lt;%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%&gt;
&lt;!DOCTYPE html&gt;
...
<p>
http://localhost:8080/sender.jsp를 방문하고 서밋 버튼을 클릭한다.<br />
이동한 taker.jsp에서 한글이 깨지지 않고 모두 출력되는지 확인한다.<br />
list.jsp에서 검색폼의 method 속성을 method="post" 에서 method="get"으로 변경한 후 한글 검색이 되는지 확인한다.<br />
</p>
