...
<meta name="Keywords" content="jdbc,JDBC,Connection Pool,Log.java,DBConnectionPool.java,DBConnectionPoolManager.java,ConnectionManager.java,OracleConnectionManager.java,oracle.properties,GetEmp.java,DBCP,BasicDataSourceExample.java" />
<meta name="Description" content="This article describes Connection Pool" />
...
<p>
Attempting to connect databases in a Java program (getting a Connection instance) is time-consuming. If you can create multiple Connections in advance and store them in the repository, you can save time to get Connection instances. This programming technique is called connection pooling.
</p>

<p>
When using connection pooling, you have to return the Connection instance to the repository instead of using the Connection's close() method.
</p>

<h3>Summary of Connection Pooling Classes</h3>

<p>
<strong>Log</strong> outputs log messages to a file.
</p>

<p>
<strong>DBConnectionPool</strong> puts database connections into a pool and manages it.
</p>

<p>
<strong>DBConnectionPoolManager</strong> manages DBConnectionPool instances.
</p>

<p>
<strong>ConnectionManager</strong> is an abstract class that asks the DBConnectionPoolManager for a connection. The reason for the abstract class is that it considers multiple databases. Depending on the database you are using, create a class that inherits this class.
</p>

<p>
<strong>OracleConnectionManager</strong> inherits the ConnectionManager to manage connections for Oracle.
</p>

<p>
The <strong>oracle.properties</strong> file is for the connection pool configuration for Oracle.
In oracle.properties, the string "oracle" is the distinguished name of the connection pool instance for Oracle.
Managing the configuration as a file is better for reuse and maintenance than implementing it as Java code.
</p>
...
<p>
Create an empty file named connection-pool.log in C:/Users/java/jdbc according to the following code in Log.java:<br /> 
public String logFile = "C:/Users/java/jdbc/connection-pool.log"<br />
Create a file named oracle.properties in C:/Users/java/jdbc according to the following code in ConnectionManager.java:<br />
configFile = "C:/Users/java/jdbc/" + poolName + ".properties";<br />
...
We will test BasicDataSourceExample.java with a little modification.
Add the commons-dbcp, commons-pool, and commons-logging libraries to the Build Path to practice the example.
</p>

<p>
Note that DBCP 2 runs on JDK 7 environment and higher.
You have to use DBCP 1.4 on JDK 6 and DBCP 1.3 on JDK 6 and below.
</p>
...
This abstract class has a method for getting a Connection instance.
</p>
...
<p>
MaxConn, initConn, maxWait If not set, the default values work.
Once you've set these up in your properties file, add the following to your code:
</p>
...
<p>
For instructions on setting these in BasicDataSource, see the official site documentation.
</p>
