...
name, position, telephone 인스턴스 변수와 이 변수에 대한 게터와 세터 메소드가 중복된다.
...
<p>
클래스가 어떤 클래스를 상속하려면 클래스 선언에 extends 키워드를 사용해야 한다.
...
관리자 클래스의 toString() 메소드에서 getName(), getPosition(), getTelephone()을 사용해야 하는 이유가 사원 클래스의
name, position, telephone의 접근자가 private이기 때문이다.
부모의 멤버변수인 name, position, telephone에 자식에서 바로 접근하려면 이들 멤버 변수의 접근자를 변경해야 하다.
사원 클래스와 관리자 클래스가 같은 패키지에 있으므로 사원 클래스의 name, position, telephone에 package private 접근자 이상으로 적용하면 된다.
...
<dd>
부모로부터 상속받은 메소드 그대로 사용할 수 있지만, 원한다면 '재정의'해서 사용하는 것을 <strong>메소드 오버라이딩</strong><a href="#comments"><sup>2</sup></a>이라고 한다.
자식 클래스에서 부모 클래스의 메소드를 재정의할 때 리턴 타입, 메소드명, 파라미터 리스트가 같아야 한다.
...
사원 클래스 코드(Employee.java)를 다시 확인해 보자. 클래스 선언에 extends 키워드가 있는가?
클래스 선언에 extends 키워드가 없다면 그 클래스는 Object 클래스를 상속하게 된다.
컴파일러가 개입하여 클래스 선언을 public class Employee extends Object로 변경하여 컴파일하기 때문이다.
결과적으로 사원 클래스의 부모 클래스는 Object가 된다.
@Override와 같은 쓰임을 어노테이션이라 하는데, 자바 코드로 전달할 수 없는 정보를 컴파일러나 플랫폼에 전달한다.
위 코드에서 @Override는 메소드가 부모 클래스의 메소드를 오버라이딩한다는 정보를 컴파일러에게 알려주게 된다.
</dd>
</dl>
...
<p>
Test 클래스의 메인 메소드에서 <em>Manager hong = new Manager();</em>는 Manager 객체를 생성하는 코드다.
이제 이 코드에 대해서 좀 더 자세히 말할 때가 되었다.
new 다음에 오는 Manager()는 메소드 모양이지만 메소드가 아닌 Manager()라는 생성자를 호출하는 코드다.
메소드와 마찬가지로 Manager() 생성자를 호출하려면, 클래스 몸체에 Manager() 생성자를 선언해야 한다.
그리고 new 다음에는 클래스에 선언된 생성자 중에 하나를 호출할 수 있다.
</p>

<p>
위의 예제에서는 사원 클래스와 관리자 클래스에 생성자를 만들지 않았다.
그렇지만 Test의 메인 메소드는 생성자를 호출한다.
그리고 에러 없이 결과를 볼 수 있다는 것은 호출한 생성자가 실행되었음을 의미한다.
만들지도 않는 생성자가 호출될 수 있을까?
답은 컴파일러에 있다. 클래스를 작성하면서 아무런 생성자도 만들지 않았다면, 컴파일러는 파라미터가 없는 생성자를 만들어 컴파일한다.
컴파일러가 자동으로 생성해 준 생성자를 '디폴트 생성자'(default constructor)라고 한다.
만일 생성자를 하나라도 만들었다면 컴파일러는 디폴트 생성자를 만들지 않는다.
</p>

<p>
다양한 파라미터 리스트를 가진 생성자를 여러개 만들 수 있다.
...
생성자가 에러없이 마치면 변수에 레퍼러스가 할당된다.
Manager <strong>hong</strong> = new Manager();에서 <strong>hong</strong>에 할당된다.
만일 생성자가 제대로 종료되지 않았다면, 변수에 레퍼런스가 할당되지 않는다.
...
<dt>this</dt>
<dd>
this가 실행되는 시점에 this는 객체 자신의 레퍼런스를 가진다.
this 키워드는 생성자 안에서 다른 생성자를 호출할 때와 생성자나 메소드 몸체에서 인스턴스 변수와 파라미터를 구별하기 위해 사용한다.
참고로 이클립스에서 코드를 작성할 때 this 다음에 .(도트)를 이어 입력하면 사용가능한 자원(멤버 변수나 메소드 등)에 대한 코드 어시스트를 받을 수 있다.
</dd>
<dt>super</dt>
<dd>
super 키워드는 메소드 오버라이딩으로 숨겨진 부모 클래스의 메소드를 호출하거나
...
<h6 class="src">테스트의 메인 메소드 구현부 수정</h6>
<pre class="prettyprint">
...
<p>
System.out.println(hong);에서 레퍼런스를 전달받는 println() 메소드는 레퍼런스가 가리키는 객체의 toString()메소드를 호출하여 반환된 문자열을 출력한다.
...
<h2>부모 타입 변수에 자식 타입 레퍼런스를 할당할 수 있다.</h2>

<p>
다형성은 같은 모양이 다양한 형태로 실행되는 듯한 느낌이 들게 한다.
부모 타입 레퍼런스 변수에 자식 타입 레퍼런스를 할당하면 다형성이 생긴다.
</p>
...
<p>
부모 클래스 타입 변수가 자식 타입의 객체를 참조할 때 객체의 모든 멤버에 접근할 수 있는 건 아니다.
제한이 있는데 자식의 고유한 멤버는 접근하지 못한다.
여기에도 예외가 있다.
자식이 오버라이딩한 메소드는 접근할 수 있다.(사실 이 부분이 가장 이해하기 힘들다.)
결론적으로, 부모 타입의 레퍼런스는 부모로부터 상속된 것과 오버라이딩 메소드를 접근할 수 있다.
예제를 통해 알아보자.
사원 클래스와 관리자 클래스는 그대로 두고 테스트의 메인 메소드에 다음을 추가한다.
</p>

<h6 class="src">Test.java 의 메인 메소드 구현부 추가</h6>
<pre class="prettyprint">
...
<p>
마지막 줄 System.out.println(jang); 여기서 jang은 Object 타입 레퍼런스다.
...
<dd>
파라미터 리스트가 다르면 똑같은 이름의 메소드를 얼마든지 만들 수 있다는 것이 메소드 오버로딩이다.
overloading이란 영어 단어의 뜻은 '과적'이다. '고속도로에서 과적 차량 단속'이라 할 때 바로 그 과적이다.
메소드 오버로딩은 똑같은 이름으로 메소드를 여러 개 만들어도 컴파일 에러가 나지 않을 뿐 아니라 호출하면서 전달한 아규먼트에 따라 거기에 꼭 맞게 선언된 메소드가 호출되어야 한다.
메소드 반환값의 타입은 메소드 오버로딩과 아무런 상관이 없다.
파라미터 리스트가 같으면서 반환값 타입만 다르게 하여 같은 이름의 메소드를 만들 수 없다.
이런 메소드를 만들면 컴파일 에러를 만나게 된다.
</dd>
<dd>
자바에서는 작명이 중요하다.
메소드 이름만 보고도 메소드가 무슨 행위를 하는지 파악이 되도록 해야 한다.
메소드 오버로딩은 이름을 짓는데 있어서 부담을 줄여준다.
</dd>
<dd>
오버로딩된 메소드를 사용하는 입장에서는 같은 모양이 다양한 형태로 실행되는 듯한 느낌을 받게 된다.
메소드 오버로딩은 다형성을 가진다.
가장 좋은 예는 System.out.println();처럼 써왔던 print()와 println() 메소드<a href="#comments"><sup>5</sup></a>다.
print()와 println() 메소드는 어떤 아규먼트라도 모두 출력하는 듯이 보인다.
사실은 print()와 println() 메소드가 파라미터 리스트가 다르게 오버로딩(과적)되어 만들어져 있고, 아규먼트에 따라 그에 맞는 메소드가 정확히 호출되는 것이다.
</dd>
</dl>

<h2>final 키워드</h2>
<ol>
	<li>클래스 선언에 사용하면, 해당 클래스를 상속하여 서브 클래스를 만들지 못한다.<a href="#comments"><sup>6</sup></a></li>
	<li>메소드 선언에 사용하면, 해당 메소드는 자식 클래스에서 오버라이딩 할 수 없다.</li>
...
<p>
클래스 선언에서 class앞에 abstract가 있는 클래스를 추상 클래스라고 한다.
추상 클래스는 일반적인 클래스와 달리 new 키워드로 객체로 생성할 수 없다.
추상 클래스를 이해하기 위해서는 먼저 추상 메소드의 의미를 알아야 한다.
추상 메소드란 메소드 선언만 있고 메소드의 몸체가 없는 메소드다.
추상 메소드는 다른 메소드와 구분하기 위해서 메소드 선언에서 class 앞에 abstract를 붙인다.
클래스에 추상 메소드가 하나라도 있다면, 그 클래스는 추상 클래스로 선언해야 한다.
반대로 모든 추상 클래스에 1개 이상의 추상 메소드가 있어야 하는 건 아니다. 필요에 따라서 추상 메소드없이 추상 클래스를 선언하기도 한다.
추상 클래스를 이용하려면 추상 클래스를 상속한 자식 클래스를 만들고 자식 클래스에서 추상 클래스의 추상 메소드를 구현해야 한다.
...
이때 사원 클래스는 AbstractEmployee클래스의 추상 메소드 doWork() 메소드를 구현해야 한다.
</p>
...
<p>
관리자 클래스는 변경하지 않는다.
지금까지 작성한 Test클래스의 메인 메소드 첫 라인을 아래처럼 변경하여 실행한다.
...
<p>
인터페이스는 클래스 선언에 class대신 interface를 쓰며 모든 메소드가 추상 메소드다.
...
인터페이스를 구현한 클래스는 해당 인터페이스의 모든 추상 메소드를 빠짐없이 구현한 클래스다.
인터페이스를 구현한 클래스의 클래스 선언은 extends 대신 <strong>implements</strong> 키워드를 사용해 클래스가 어떤 인터페이스를 구현했는지를 정의한다.
...
자바 소스 차원에서 보면 클래스 선언에는 extends 다음에 단 하나의 클래스만 둘 수 있다. extends 다음에 ,(콤마)로 부모 클래스를 나열할 수 없다.
...
<p>
상속에서 확인했던 '부모 타입 레퍼런스 변수에 자식 타입 레퍼런스가 할당될 수 있다'와 같이
'인터페이스 타입의 변수에 인터페이스를 구현한 클래스의 레퍼런스가 할당될 수 있다.'
</p>
...
<ol>
	...
	<li>생성자가 호출되기 전 객체의 인스턴스 변수 값은, 숫자 타입은 0으로, 불린은 false로, 레퍼런스 변수는 null로 초기화된다.
	char도 엄밀하게는 숫자 타입으로 유니코드 테이블에서 0에 해당하는 문자로 초기화된다.</li>
	<li>print()와 println() 메소드는 java.io.PrintStream 클래스의 메소드다.</li>
