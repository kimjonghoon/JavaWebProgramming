...
<head>
<title>Model 2</title>
<meta name="Keywords" content="java,model 2,el,jstl,board program," />
<meta name="Description" content="This article describes how to convert model 1 board program to model 2" />
...
<p>
To modify our project with Model 2, you first need to know about EL and JSTL.
</p>
...
<p>
In Model 2, views only serve to display the received data. Models generate data. Therefore, you should avoid using scriptlets in Model 2 as much as possible.
</p>

<h3>JSTL</h3>

<p>
A tag library is a technique for creating tags that change into Java code. JST Standard Tag Library (JSTL) provides a standard for tag libraries. JSP Spec does not include JSTL. Therefore, you need to download the library to use JSTL. Download and extract the most recent version of JSTL from <a href="http://archive.apache.org/dist/jakarta/taglibs/standard/binaries/">http://archive.apache.org/dist/jakarta/taglibs/standard/binaries/</a> and Copy jstl.jar and standard.jar from the lib folder to WEB-INF/lib of out project. The core of JSTL is the most used. To use the core, add the taglib directive to the JSP like below.
</p>
...
<p>
Let's test a simple JSTL example. Create the following file on the document base.
</p>
...
<p>
The example shows the basics of the JSTL core.
</p>

<p>
Now you are ready to change the project from Model 1 to Model 2.
Significant changes are as follows.
</p>

<ul>
	<li>All JSPs use EL and JSTL.</li>
	<li>JSP only shows the received data.</li>
	<li>Action classes are responsible for business logic.</li>
	<li>Requests with the extension *.do are sent to the controller.</li>
	<li>Change the parameter name curPage, which contains the current page number, to page.</li>
</ul>

<p>
Use EL and JSTL to modify JSPs and change all .jsp to .do, except for the jsp used in the include directive.
</p>
...
&lt;ul&gt;
  &lt;li&gt;
    &lt;ul&gt;
      &lt;li&gt;&lt;a href="list<strong>.do</strong>?boardCd=chat&amp;page=1"&gt;Small talk&lt;/a&gt;&lt;/li&gt;
...
<p>
Modify also the main-menu.jsp in the same way.
</p>

<p>
Delete the loginCheck.jsp file before modifying the list page. We will delegate what the loginCheck.jsp did to an action class.
</p>
...
<p>
You need to create an action class building the following data and passing them to the list page.
</p>
...
<p>
You need to create an action class building the following data and passing them to the view page.
</p>
...
<p>
Change the file name of the write_form.jsp to write.jsp and modify it like below.
</p>
...
<p>
you need to create an action class building the board's name and passing it to the write form page.
</p>

<p>
Change the name of modify_form.jsp to modify.jsp and modify it like below.
</p>
...
<p>
You need to create an action class building the following data and passing them to the modify.jsp.
</p>
...
<p>
Let's work with membership-related views.
</p>
...
<p>
This single error page covers the main errors. For easy implementation, this page does not use JSTL.
</p>
...
<p>
Don't forget to edit the homepage as well.
</p>

<h3>Implementation</h3>

<p>
ActionForward is the return type of the execute() method of the action class. ActionForward object stores only the address to forward or redirect. If the ActionForward object's redirect property is false, forwarding works. Action classes are responsible for making addresses, storing them in ActionForward, and setting the redirect flag of ActionForward.
</p>
...
<p>
The below is a custom exception class to use when authentication fails. It inherits runtime exceptions.
</p>
...
<p>
Our controller handles * .do requests.
</p>
...
<p>
Declare the controller servlet in web.xml and map it to the specific pattern URL.
</p>
...
<p>
Since we have established a policy that only logged-in users can use the bulletin board, we will implement login, our top priority. Refer to Model 1's implementation of login_proc.jsp to create an action class to handle logins. This class works when a login.do POST request comes in. If login fails, go back to the login page.
</p>
...
<p>
You have to obtain the address for redirecting and the address for fowarding differently. Addresses for forwarding use addresses starting with "/" excluding the context path. Addresses for redirecting uses an address beginning with the context path or uses a relative path based on the page's location that sends the redirect URL. Since our project is a root application, it is the same whether we add a context path to the address we want to redirect to or not.
</p>
...
<p>
SignUpAction handles subscription.
</p>
...
<p>
EditAccountFormAction's logic runs when the /users/editAccount.do request is passed to the controller by clicking the Edit My Info button.
</p>
...
<p>
EditAccountAction's logic acts on the /users/editAccount.do POST request to edit the user's information.
</p>
...
<p>
ChangePasswdFormAction's logic acts on the /users/changePasswd.do request.
</p>
...
<p>
ChangePasswdAction works on the /users/changePasswd.do POST request.
</p>
...
<p>
ByeFormAction works on the /users/bye.do GET request.
</p>
...
<p>
ByeAction works on the /users/bye.do POST request.
</p>
...
<p>
ListAction works on the /bbs/list.do GET request.
</p>
...
<p>
ViewAction works on the /bbs/view.do GET request.
</p>
...
<p>
WriteFormAction works on the /bbs/write.do GET request.
</p>
...
<p>
WriteAction works on the /bbs/write.do POST request.
</p>
...
<p>
ModifyFormAction works on the /bbs/modify.do GET request.
</p>
...
<p>
ModifyAction works on the /bbs/modify.do POST request.
</p>
...
<p>
DeleteAction works on the /bbs/del.do POST request.
</p>
...
<p>
AddCommentAction works on the /bbs/addComment.do POST request.
</p>
...
<p>
UpdateCommentAction works on the /bbs/updateComment.do POST request.
</p>
...
<p>
DeleteCommentAction works on the /bbs/deleteComment.do POST request.
</p>
...
<p>
DeleteAttachFileAction works on the /bbs/deleteAttachFile.do POST request.
</p>
...
<ul>
	<li>All requests go to the controller.</li>
	<li>The controller delegates tasks to the model.</li>
	<li>The model performs the business logic and passes the results to the view.</li>
</ul>

<p>
Business logic is the logic that processes data required for business.
Models are responsible for business logic.
Views display the data that models generate, and the controller passes.
Views in web applications are JSPs.
</p>

<p>
Business logic needs to be detached from views and implemented in models.
If a view has business logic, it's not good code.
</p>
