...
<head>
<title>게시판 구현</title>
<meta name="Keywords" content="java,board program,implementation," />
<meta name="Description" content="게시판 구현" />
...
<p>
http://localhost:8080/users/signUp.jsp를 방문하여 테스트한다.<br />
...
로그 메시지는 CATALINA_HOME/logs 디렉터리에 있는 파일에도 쌓이니 참고한다.<br />
...
<p>
목록 /bbs/list.jsp를 구현한다.
내용이 많으니 나누어 진행하겠다.
전달된 파라미터를 받아서 목록에서 보일 데이터를 만든다.
</p>
...
%&gt;</strong>
        &lt;a href="javascript:goList('&lt;%=numbers.getPrevPage() %&gt;')"&gt;[이전]&lt;/a&gt;
<strong>&lt;%
} // if 문 끝
...
%&gt;</strong>
    &lt;a href="javascript:goList('&lt;%=numbers.getNextPage() %&gt;')"&gt;[다음]&lt;/a&gt;
<strong>&lt;%
}// if문 끝
%&gt;</strong>
    &lt;/div&gt;
...
<h3>글쓰기 처리</h3>
<p>
글쓰기 처리 페이지 write_proc.jsp를 구현한다.
파일 업로드를 위한 라이브러리가 필요하다.
<a href="http://www.servlets.com/cos/cos-26Dec2008.zip">http://www.servlets.com/cos/cos-26Dec2008.zip</a>를 내려받고 압축을 푼다.
웹 애플리케이션의 WEB-INF/lib에 cos.jar를 복사한다.
도큐먼트 베이스 아래 upload라는 폴더를 만든다.
유닉스 계열 시스템이면 upload 폴더에 권한 설정이 필요하다.
</p>
...
List&lt;Article&gt; articleList = service.getArticleList(boardCd, searchWord, startRecord, endRecord);//게시글 목록 
List&lt;Comment&gt; commentList = service.getCommentList(articleNo);//댓글 목록

String boardName = service.getBoardName(boardCd);//게시판 이름
...
댓글을 인서트 한 후 상세보기로 돌아가기 위해 검색어 searchWord를 URLEncoder의 encode() 메소드로 UTF-8으로 인코딩한다.
*/
<strong>
User user = (User) session.getAttribute(WebContants.USER_KEY);
if (user == null) {
...
댓글을 업데이트한다.
댓글을 업데이트한 후 상세보기로 돌아가기 위해 검색어 searchWord를 URLEncoder의 encode() 메소드로 UTF-8으로 인코딩한다.
*/
<strong>
User user = (User) session.getAttribute(WebContants.USER_KEY);
...
댓글을 삭제 후 상세보기를 돌아가기 위해선 searchWord를 URLEncoder의 encode() 메소드로 UTF-8으로 인코딩한다.
*/
<strong>
User user = (User) session.getAttribute(WebContants.USER_KEY);
...
첨부 파일을 삭제 후 상세보기를 돌아가기 위해 searchWord를 URLEncoder의 encode() 메소드로 UTF-8으로 인코딩한다.
*/
<strong>
User user = (User) session.getAttribute(WebContants.USER_KEY);
...
게시글을 수정한 후 상세보기를 돌아가기 위해 검색어 searchWord를 URLEncoder의 encode() 메소드로 UTF-8으로 인코딩한다.
*/
<strong>
User user = (User) session.getAttribute(WebContants.USER_KEY);
...
게시글 삭제 후 목록을 돌아가기 위해 검색어 searchWord를 URLEncoder의 encode() 메소드로 UTF-8으로 인코딩한다.
*/
<strong>
User user = (User) session.getAttribute(WebContants.USER_KEY);
