...
<p>
데이터베이스 벤더들이 이러한 라이브러리를 각자 만든다면,
자바 프로그래머는 각 벤더가 만든 라이브러리 사용법을 익혀야 한다.<br />
</p>

<p>
JDBC는 선에서 만든, RDBMS에 접근하여 SQL문을 실행하기 위한, 자바 라이브러리를 말한다.
JDBC에는 구현클래스가 거의 없고 대부분이 인터페이스다.
(그래서 표준이란 표현을 쓴다)
인터페이스를 구현한 클래스는 각 벤더가 만들어야 한다.<br />
JDBC 예제 소스를 보면 각 벤더가 만든 JDBC 구현체는 보이지 않는다.
선(Sun)의 인터페이스만 보인다.
</p>
