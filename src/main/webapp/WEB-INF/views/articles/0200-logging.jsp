...
파일 압축을 풀면 생기는 디렉터리에서 log4j-api-2.x.x.jar 와 log4j-core-2.x.x.jar을 복사하고 클래스로더가 찾을 수 있는 경로에 붙여넣는다.<br />
...
<ol>
	...
	<li>Log4j 2 라이브러리를 프로젝트 빌드패스에 추가한다.
		<ol>
			...
			<li>log4j-api-2.x.x.jar 와 log4j-core-2.x.x.jar를 추가하고 OK 버튼을 클릭한다.</li> 
		</ol>
...
<h3>Commons Logging와 Log4j 2 연동</h3>
...
Account, NormalAccount, MinusAccount 클래스의 입금과 출금 메소드에 로깅을 적용한다.<br />
</p>
...
public synchronized void withdraw(double amount) {
	...
	transactions.add(transaction);
	<strong>logger.debug("AccountNo:{} Amount:{} DEPOSIT/WITHDRAW:{} NORMAL/MINUS:{}", 
		getAccountNo(), amount, Account.WITHDRAW, this.getKind());</strong>
}
</pre>
