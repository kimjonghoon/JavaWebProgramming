...
<meta name="Description" content="모델 1 게시판을 모델 2 게시판으로 변경하기" />
...
<p>
모델 2 방식으로 게시판을 변경하려면 먼저 EL과 JSTL에 대한 학습이 필요하다.<br />
...
<p>
다음은 EL에서 사용할 수 있는 내재 객체 리스트다.<br />
</p>
...
<p>
모델 2에서 뷰는 전달받은 데이터를 보여주는 역할만을 수행한다.<br />
데이터를 생산하는 역할은 모델이 담당하도록 해야 한다.<br />
...
하지만 너무 많이 만들어져 부작용이 부각되었다.<br /> 
JSTL은 JSP Standard Tag Library의 약자로 태그 라이브러리의 표준을 제공한다.<br />
JSTL은 자바 프로그래머로 하여금 새로운 태그 라이브러리를 만들어야 한 개연성을 줄여준다.<br />
예를 들어, 당신이 게시판 목록을 출력하기 위한 태그 라이브러리를 생각하고 있다면, 먼저 JSTL의 forEach 반복자를 고려해 봐야 한다.<br />
(이 경우 JSTL의 forEach 반복자를 사용하는 게 재사용과 유지보수 측면 모두에서 유리하다)
JSTL은 JSP 스펙에 포함되어 있지 않다.<br />
따라서 JSTL 사용하려면 라이브러리를 내려받아야 한다.<br />
다음 주소에서 가장 최신 버전인 JSTL 1.1.2를 내려받는다.<br /> 
<a href="http://archive.apache.org/dist/jakarta/taglibs/standard/binaries/">http://archive.apache.org/dist/jakarta/taglibs/standard/binaries/</a>
...
<ul>
  ...
  <li>페이지 번호를 담는 파라미터 이름 curPage를 page로 바꾼다.</li>
</ul>
...
인클루드 지시어에 사용된 jsp를 제외한 모든 .jsp를 .do로 바꾼다.
</p>
...
<strong>&lt;c:choose&gt;
    ...
    <strong>&lt;/c:when&gt;
    &lt;c:otherwise&gt;</strong>
        &lt;input type="button" value="로그아웃" onclick="location.href='/users/logout<strong>.do</strong>'" /&gt;
...
&lt;ul&gt;
    &lt;li&gt;
        &lt;ul&gt;
            &lt;li&gt;&lt;a href="list<strong>.do</strong>?boardCd=chat&amp;page=1"&gt;자유 게시판&lt;/a&gt;&lt;/li&gt;
...
<ul>
	<li>boardNm: 게시판 이름</li>
	<li>list: 게시글이 담긴 ArrayList</li>
	<li>listItemNo: 목록 아이템에 붙는 번호</li>
	<li>prevPage: [이전] 링크를 위한 페이지 번호</li>
	<li>firstPage: 현재 블록에 해당하는 첫 번째 페이지 번호</li>
	<li>lastPage: 현재 블록에 해당하는 마지막 페이지 번호</li>
	<li>nextPage: [다음] 링크를 위한 페이지 번호</li>
</ul>
...
<ul>
	<li>boardNm: 게시판 이름</li>
	<li>title: 상세보기 대상 게시글의 제목</li>
	<li>regdate: 상세보기 대상 게시글의 등록일</li>
	<li>name: 상세보기 대상 게시글의 소유자 이름</li>
	<li>hit: 상세보기 대상 게시글의 조회 수</li>
	<li>content: 상세보기 대상 게시글의 내용</li>
	<li>attachFileList: 상세보기 대상 게시글의 첨부 파일 리스트</li>
	<li>commentList: 상세보기 대상 게시글의 댓글 리스트</li>
	<li>nextArticle: 다음 게시글 객체</li>
	<li>prevArticle: 이전 게시글 객체</li>
	<li>-- 이하는 목록과 같음 --</li>
	<li>list: 목록</li> 
	<li>listItemNo: 목록 아이템에 붙일 번호</li>
	<li>prevPage: [이전] 링크를 위한 페이지 번호</li>
	<li>firstPage: 현재 블록에 속한 첫 번째 페이지 번호</li>
	<li>lastPage: 현재 블록에 속한 마지막 페이지 번호</li>
	<li>nextPage: [다음] 링크를 위한 페이지 번호</li>
</ul>
...
<p>
하나의 에러 페이지로 모든 에러를 처리하기로 했으므로 굳이 JSTL로 수정할 필요가 없다.<br />
...
%&gt;
    ...
    &lt;div id="main-menu"&gt;
        &lt;ul id="nav"&gt;
            ...
            &lt;li&gt;&lt;a href="/bbs/list<strong>.do</strong>?boardCd=chat&amp;page=1"&gt;BBS&lt;/a&gt;&lt;/li&gt;
        &lt;/ul&gt;
    &lt;/div&gt;
...
아래 ActionForward 클래스는 액션 클래스의 execute() 메소드가 반환하는 타입이다.<br />
ActionForward 객체에는 리다이렉트 여부와 이동하게 될 주소 정보만 저장된다.<br />
리다이렉트 여부가 false면 설정된 주소로 포워딩된다.<br />
...
따라서 로그인을 첫 번째로 구현한다.<br />  
모델 1의 login_proc.jsp의 구현 내용을 참조하여 로그인을 처리하는 액션 클래스를 만든다.<br />
이 클래스는 login.do POST 요청이 왔을 때 작동한다.<br />
...
<p>
EditAccountAction 액션 클래스는 /users/editAccount.do POST 요청에 대해 작동하여
회원정보 수정을 처리한다.
</p>
...
<p>
ChangePasswdAction 액션 클래스는 /users/changePasswd.do POST 요청에 작동하여 
비밀번호 변경을 처리한다.
</p>
...
<p>
ListAction 액션 클래스는 /bbs/list.do GET 요청에 작동한다.<br />
...
<p>
ViewAction 액션 클래스는 /bbs/view.do GET 요청에 작동한다.
...
<p>
WriteFormAction 액션 클래스는 /bbs/write.do GET 요청에 작동한다.<br />
</p>
...
<p>
WriteAction 액션 클래스는 /bbs/write.do POST 요청에 작동하여 글쓰기를 처리한다.<br />
</p>
...
<p>
ModifyFormAction 액션 클래스는 /bbs/modify.do GET 요청에 작동한다.<br />
</p>
...
<p>
ModifyAction 액션 클래스는 /bbs/modify.do POST 요청에 작동하여 글 수정을 처리한다.<br />
</p>
...
<p>
DeleteAction 액션 클래스는 /bbs/del.do POST 요청에 작동하여 게시글 삭제를 처리한다.<br />
</p>
...
<p>
AddCommentAction 액션 클래스는 /bbs/addComment.do POST 요청에 작동하여 댓글 쓰기를 처리한다.<br />
</p>
...
<p>
UpdateCommentAction 액션 클래스는 /bbs/updateComment.do POST 요청에 작동하여 댓글 수정을 처리한다.<br />
</p>
...
<p>
DeleteCommentAction 액션 클래스는 /bbs/deleteComment.do POST 요청에 작동하여 댓글 삭제를 처리한다.<br />
</p>
...
<p>
뷰는 프로젝트에서 JSP가 담당한다.<br />
모델은 프로젝트에서 액션 클래스이며 비즈니스 로직을 담당한다.<br />
