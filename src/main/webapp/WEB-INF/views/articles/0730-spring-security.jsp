...
<ol>
  ...
  <li>스프링 시큐리티 설정 파일 작성</li>
  <li>web.xml 파일에 시큐리티 설정 추가</li>
  <li>기존 기존 로그인/로그아웃 메소드 제거</li>
...
<p>
스프링 시큐리티를 사용하기 위해서 회원과 권한 테이블이 필요하다.<br />
...
<h3>3. 스프링 시큐리티 설정 파일 작성</h3>

<p>
스프링 시큐리티만을 위한 스프링 설정 파일이 필요하다.<br />
이름과 위치에 제약은 없다.<br /> 
...
<p>
&lt;form-login&gt; 요소의 login-page 속성 기본값은 /login,<br />
login-processing-url 속성 기본값은 POST /login,<br />
username-parameter 속성 기본값은 username,<br />
password-parameter 속성 기본값은 password,<br />
authentication-failure-url 속성 기본값은 /login?error=1이다.<br />
기본값 이외의 값을 주려면 속성을 생략해선 안 된다.<br />
...
<h3>7. 로그인 페이지 수정과 로그아웃 링크 수정</h3>
...
스프링 시큐리티 태그를 사용하는 예는 뷰 레벨 보안에서도 다룬다.
로그아웃을 위한 메뉴는 제거한다.
</p>

<h3>8. 권한이 없는 사용자가 권한이 필요한 자원을 요청했을 때 보여줄 페이지</h3>
...
<h3>9. 스프링 시큐리티를 적용하기 위해 필요한 코드 추가</h3>
...
<p>
라이브러리가 추가했으니 다시 빌드를 실행한다.<br />
톰캣을 재실행한 후, http://localhost:8080/list?boardCd=chat&amp;page=1를 방문한다.<br />
로그인 페이지로 이동하게 되는데, im@gmail.org/1111로 로그인한다.<br />
...
<img alt="Before applying spring security at viewlayer" src="<c:url value="/resources/images/view-layer-security.png"/>">
</p>
