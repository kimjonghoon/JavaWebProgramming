...
서블릿은 HTML 디자인을 자바 문자열로 만들어서 출력스트림의 메소드에 아규먼트로 전달해야만 한다.
...
톰캣은 이 서블릿을 컴파일하고, 서블릿 바이트코드로부터 객체를 생성하고, 생성한 서블릿 객체의 서비스 메소드를 호출한다.
/hello.jsp 요청이 다시 오면, 톰캣은 hello.jsp 파일이 변경되었는지를 체크한다.
hello.jsp가 변경되지 않았다면, 톰캣은 서블릿 객체가 이미 로딩되어 있는지 확인한다.
만일 객체가 메모리에 있다면, 톰캣은 서블릿 객체의 서비스 메소드를 호출한다.
...
<p>
선언은  JSP 페이지내에서 서블릿 클래스의 변수와 메소드를 선언하기 위해선 사용된다.
...
값을 변경하고자 하는 빈 속성
property="*"라면, HTTP 요청과 함께 전달된 파라미터의 이름과 매칭되는 모든 setter 메소드를 호출하여 빈의 속성값을 수정한다.
하지만 파라미터값이 ""라면 이에 상응하는 빈의 속성은 수정되지 않는다.
	</td>
</tr>
<tr>
...
<p>
내재 객체는 JSP에서 사용되는 객체로 레퍼런스를 얻기위한 작업없이 즉시 사용할 수 있다.
</p>

<h3 id="out">5.1 out</h3>

<p>
jakarta.servlet.jsp.JspWriter 추상 클래스 타입 인스턴스의 레퍼런스이다.
...
<p>
jakarta.servlet.http.HttpServletRequest 인터페이스 타입 인스턴스의 레퍼런스이다.
...
<p>
jakarta.servlet.http.HttpServletResponse 인터페이스 타입 인스턴스의 레퍼런스이다.
</p>

<h3 id="pageContext">5.4 pageContext</h3>

<p>
jakarta.servlet.jsp.PageContext 타입 인스턴스의 레퍼런스이다.
...
<p>
session 내재 객체는 서블릿의 jakarta.servlet.http.HttpSession 타입 인스턴스의 레퍼런스이다.
...
<p>
jakarta.servlet.ServletContext 인터페이스 타입 인스턴스의 레퍼런스이다.
</p>

<h3 id="config">5.7 config</h3>

<p>
jakarta.servlet.ServletConfig 인터페이스 타입 인스턴스의 레퍼런스이다.
...
<p>
page는 페이지 인스턴스를 참조한다.
이 내재 겍체 때문에 스크립틀릿에서 page라는 이름의 변수를 선언할 수 없다.
거의 쓰이지 않는다.
</p>
...
서블릿 컨텍스트는 웹 애플리케이션의 서버 측 컴포넌트와 서블릿 컨테이너와의 통신을 담당하는 메소드를 가지고 있다.
...
<p>
web.xml 파일에 HTTP 상태코드<sup>3</sup>와 발생한 익셉션 타입 별로 각각의 에러 페이지를 지정해 줄 수 있다.
...
쿠키의 유효기간을 setMaxAge() 메소드를 사용하여 구체적인 시간을 명시한다면, 응답 데이터를 받은 웹 브라우저는 응답 데이터에서 쿠키를 꺼내 쿠키저장소에 보관한다.
...
<dl class="api-summary">
	<dt class="api-summary-dt bottom-line">jakarta.servlet.http.Cookie 클래스</dt>
...
<p>
<a href="https://github.com/kimjonghoon/example">https://github.com/kimjonghoon/example</a> 링크에 있는 파일을 ROOT 애플리케이션의 최상위 디렉토리에 복사한다.
...
<p>
login_proc.jsp에서 id 파라미터의 값은 내재 객체 request의 getParameter() 메소드를 사용해서 구할 수 있다.
...
jsp:setProperty 액션은 HTTP 요청안의 파라미터값을 가지고 JSP 빈즈의 setter 메소드를 호출하면서 멤버 변수를 셋팅한다.
</p>
...
setter 메소드 이름과 매칭되는 HTTP 요청 파라미터값이 setter 메소드의 아규먼트로 전달된다.
</p>
