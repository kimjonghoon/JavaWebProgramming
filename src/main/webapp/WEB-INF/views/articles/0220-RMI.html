...
<head>
<title>RMI</title>
<meta name="Keywords" content="리모트 인터페이스,Remote Interface,Server,Client,rmiregistry,stub,RMI Runtime,1099,RemoteException" />
<meta name="Description" content="RMI 프로그래밍을 다룹니다" />
...
<p>
일반적으로 객체의 메소드 호출은 같은 JVM 환경의 객체와 객체 사이에서 이루어진다.
다른 JVM에서 동작하고 있는 객체의 메소드를 호출할 수 있을까?
우리는 이미 소켓에 대해 알고 있다.
소켓을 이용한다면 서버와 클라이언트가 사용하는 프로토콜을 개발해야 하지만
RMI를 이용한다면 프로토콜을 정해야 하는 수고 없이 다른 JVM에서 활동하고 있는 객체의 메소드를 호출할 수 있다.
...
<ul>
	...
	<li>Client.java - 원격 인터페이스의 원격 메소드를 호출하는 클라이언트</li>
</ul>
...
원격 인터페이스의 원격 메소드는 throws java.rmi.RemoteException을 선언해야 한다.
...
<p>
LocateRegistry의 getRegistry() 메소드가 아규먼트 없이 호출된다면 디폴트 포트인 1099를 사용한다.
...
<div class="cmd-header">&nbsp;</div>
<pre class="cmd">java -classpath /home/kim/java/rmi/bin example.hello.Client
</pre>
...
<div class="cmd-header">&nbsp;</div>
<pre class="cmd">java -classpath /home/kim/java/rmi/bin example.hello.Client 192.168.0.8
</pre>
